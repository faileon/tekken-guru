...
import {
  ...
  shouldFilterByProperties,
  shouldFilterCounterFrame,
...
export class MoveService implements OnDestroy {
  ...
  constructor(private characterService: CharacterService) {
    ...
    this._startUpFilter = new BehaviorSubject({
    ...
    this._blockFilter = new BehaviorSubject<NumberRange>({
    ...
    this._normalHitFilter = new BehaviorSubject<NumberRange>({
    ...
    this._counterHitFilter = new BehaviorSubject<NumberRange>({
  ...
  public getMovelist$(characterId: string): Observable<Move[]> {
    return combineLatest([
      ...
      switchMap(
        ([
          [startUpRange, blockRange, normalRange, counterRange],
          [
            blockFilterType,
            normalHitFilterType,
          ...
          [blockProps, normalProps, counterProps, moveProps, hitLevels],
          [searchText],
        ]) =>
          this.characterService.getMoves(characterId).pipe(
            ...
            map((data) => {
              ...
              return data.filter((move) => {
                ...
                const { _id, notation, notes } = move;
                const { startUp, onHit, onCounterHit, onBlock } =
                  move.frameData;

                if (withText) {
                  // notation search - strip both texts of [whitespace, /, + and ,] then look for the parts in the notation text
                  const reg = /[\s\/+,]/g;
                  const strippedNotation = notation
                    .replace(reg, '')
                    .toLowerCase();
                  const strippedSearchText = searchText
                    .replace(reg, '')
                    .toLowerCase();
                  // replace ss,ws,wr,<stance name> to full name
                  const replacedSearchText =
                    replaceAbbreviations(strippedSearchText);
                  const isPartOfNotation =
                    strippedNotation.includes(replacedSearchText);
                  ...
                  const includesHitLevel = joinedHitLevels.includes(
                    strippedSearchText.toLowerCase(),
                  );

                  const isNoteText = notes
                    ? notes
                        .replace(reg, '')
                        .toLowerCase()
                        .includes(strippedSearchText.toLowerCase())
                    : false;

                  // combine it with elastic
                  satisfiesFilter.push(
                    /*searchedMoveIds.includes(_id) ||*/
                    isPartOfNotation || includesHitLevel || isNoteText,
                  );
                }
                ...
                if (byBlockProps) {
                  satisfiesFilter.push(
                    satisfiesPropertyFilter<HitProperty>(
                      blockProps,
                      onBlock.property,
                      blockFilterType,
                    ),
                ...
                if (byMoveProps) {
                  satisfiesFilter.push(
                    satisfiesPropertyFilter<MoveProperty>(
                      moveProps,
                      move.properties,
                    ),
                  );
                ...
                if (byHitLevels) {
                  satisfiesFilter.push(
                    satisfiesPropertyFilter<HitLevel>(
                      hitLevels,
                      move.hit.move,
                      hitLevelsFilterType,
                    ),
                ...
                return satisfiesFilter.length > 0
                  ? satisfiesFilter.every((sf) => sf)
                  : true;
              });
            }),
          ),
